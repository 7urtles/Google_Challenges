...
def solution(hallway):
    ...
    position = 0
    while position < len(hallway):
        # find left most minion walking to the right
        if hallway[position] == walking_right:
            # for every space to the right of the minion
            for number in range(position, len(hallway)):
                # if it contains a left walking minion
                if hallway[number] == walking_left:
                    # add a solute for each minion
                    salutes += 2
        position += 1
    return salutes
